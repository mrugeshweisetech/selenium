...
public class Homeimagetest {
    ...
    @Test
    public void checkBrokenImagesOnHomePage() {
        test = extent.createTest("Broken Image Check on Homepage");
            ...
            for (int i = 0; i < images.size(); i++) {
                try {
                    ...
                    if (imgSrc == null || imgSrc.isEmpty()) {
                        test.warning("️ Image src is empty or missing.");
                        continue;
                    }
                    ...
                    if (responseCode >= 400) {
                        brokenCount++;
                        String ssPath = takeScreenshot("broken_image_" + i);
                    ...
                    String ssPath = takeScreenshot("exception_image_" + i);
                    test.fail(" Exception while checking image " + i + ": " + e.getMessage(),
                            MediaEntityBuilder.createScreenCaptureFromPath(ssPath).build());
                }
            }

            if (brokenCount == 0) {
                test.pass(" All images on homepage are loading correctly.");
            } else {
                test.warning("⚠️ Total broken images found: " + brokenCount);
            }

        } catch (Exception e) {
            String ssPath = takeScreenshot("homepage_exception");
